...
void MyStrategy::Move(Tank self, World world, model::Move& move) 
{
	...
	for (size_t i = 0; i < EnemiesToAttack.size(); ++i)
	{
		...
		if (dist && dist > max_dist_alive)	//если ближний и живой
			max_dist_alive = dist;
		double angle = fabs (self.GetAngleTo (all_tanks[EnemiesToAttack[i]]));
		if (angle < min_angle_alive)	//если угол меньший, и он живой
		{
	...
	{
		double dist = self.GetDistanceTo(all_tanks[EnemiesToAttack[i]]);
		double angle = fabs (self.GetTurretAngleTo(all_tanks[EnemiesToAttack[i]]));
	...
	if (max_angle_alive_enemy != 0 && max_dist_alive_enemy != 0)
	{
		for (size_t i = 0; i < EnemiesToAttack.size(); ++i)
		{
			...
			Tank enemy_obj = all_tanks[EnemiesToAttack[i]];
	...
	if (enemy && world.tick() <= self.reloading_time() * 0.8)
	{
	...
	if (goal)
	{
		...
		if (max_angle_shells && max_dist_shells)
		{
			for (size_t i = 0; i < shells.size(); ++i)
			{
				...
				double dist = shells[i].GetDistanceTo(self);
				double angle = fabs(shells[i].GetAngleTo (self));
				if (dist * sin(angle) <= self.width()/2)
					measure = 1;
